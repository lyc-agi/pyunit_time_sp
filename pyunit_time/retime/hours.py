...
class Hours(IObserver):
    ...
    def set_shift_hour(self):
        ...
        # print(match)
        # if match and self.time.hour == self.current_time.hour:  # 保证时间没有被改变过，lyc修改点20220429#and self.time.hour == self.current_time.hour
        #     hour = DayHour[match.group()]
        #     # self.time = self.time.shift(hours=hour)#不需要偏移，lyc修改点20220429
        # else:
        #lyc修改点20220707，取消和DayHour的匹配
        h = self.time.hour
